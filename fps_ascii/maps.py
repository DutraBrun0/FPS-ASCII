"""LAN house geometry, ASCII decoration, collision and grid navigation."""
...
class WorldMap:
    """A connected LAN house; '#' walls, 'C' desks and 'S' server racks.

    Ray distances are parameters along the supplied direction, which lets the
    renderer use camera-plane rays without an extra fish-eye correction.
    """
    ...
    def __init__(self) -> None:
        ...
        # Two-cell doorways make movement comfortable even while sliding.
        for x in (8, 16):
            for y in range(2, 22):
        ...
        # Computer islands leave walkable routes on every side.
        ...
        # Spawn only in open floor, with enough clearance for large enemies.
        self.spawn_points = [
            (x + 0.5, y + 0.5)
    ...
    def collides(self, x: float, y: float, radius: float = 0.22) -> bool:
        """Circle-to-cell collision, including solid space outside the map."""
        if not math.isfinite(x) or not math.isfinite(y):
            return True
    ...
    def move(self, entity: object, dx: float, dy: float, radius: float = 0.22) -> None:
        """Resolve each axis independently, substepping to prevent tunnelling."""
    ...
    def raycast(
        ...
        """DDA grid traversal returning (ray parameter, wall side, cell x, cell y)."""
    ...
    def path_direction(self, x: float, y: float, tx: float, ty: float) -> tuple[float, float]:
        """Reuse one reverse BFS until the target moves into another map cell."""
        ...
        if start == target:
            ...
            if not candidates:
                return 0.0, 0.0
            # Stable tie-breaking favors the player's real position.
