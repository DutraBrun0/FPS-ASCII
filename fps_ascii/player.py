"""Player movement and health independent of the display/input backend."""
...
class Player:
    ...
    def update(self, dt: float, controls: dict, world: WorldMap) -> None:
        """Apply WASD movement. Jump is an input edge; crouch starts a slide.

        Mouse deltas belong to the frontend, which owns angle and pitch.
        The frontend should clamp frame dt; movement still uses collision
        substeps so a delayed frame cannot carry the player through a wall.
        """
        if dt <= 0:
            return
        if not self.alive:
        ...
        if self.slide_timer > 0:
            # Decay from a sharp burst to walking speed over the slide.
        ...
        # Semi-implicit gravity; the landing clamp prevents accumulation below 0.
